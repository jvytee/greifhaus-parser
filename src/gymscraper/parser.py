...
def parseTarget(target, outputDir):
    ...
    if currentVisitors is None or currentFree is None:
        Log.log(
            Log.error,
            "Failed to parse: currentVisitors = {}, currentFree = {}".format(
                currentVisitors, currentFree
            ),
        )
        exit(-1)
...
def loadConfig():
    ...
    if not os.path.isfile(configFilename):
        Log.log(
            Log.error,
            'File "{0}" not found. Creating empty config file, please fill in the empty fields'.format(
                configFilename
            ),
        )
    ...
    with open(configFilename, "r") as configFile:
        try:
            ...
            Log.log(
                Log.error,
                "Corrupt config. Creating empty config file, please fill in the empty fiels",
            )
            createEmptyConfig(configFilename, defaultConfig)
            return None
    ...
    for key in defaultConfig:
        if not key in config or type(defaultConfig[key]) is not type(config[key]):
            Log.log(
                Log.error,
                'File "mail.config" incomplete. {0} is missing or invalid. Renaming old config file and generating new config'.format(
                    key
                ),
            )
            createEmptyConfig(configFilename, defaultConfig)
